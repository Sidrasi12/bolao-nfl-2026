Bolao.LiveGames = {
  ...
  displayedSpread(game, reveal) {
    const current = Bolao.ESPN.spreadText(game);
    if (current && current !== 'Spread indisponível') return current;
    return reveal?.spread || 'Spread indisponível';
  },

  displayedFavorite(game, reveal) {
    const current = Bolao.ESPN.favorite(game);
    if (current && current !== 'Não definido') return current;
  ...
  html(game, reveal, locked) {
    ...
    let html = `<section class="score-panel ${game.state === 'in' ? 'is-live' : ''}">
      <div class="score-panel-title"><span>PLACAR</span>${game.state === 'in' ? '<b>AO VIVO</b>' : ''}</div>
      <div class="score-row"><span class="score-team">${game.away.abbr}</span><strong>${game.away.score}</strong><span class="score-x">×</span><strong>${game.home.score}</strong><span class="score-team">${game.home.abbr}</span></div>
      ${status ? `<div class="score-status">${status}</div>` : ''}
    </section>
    <section class="odds-card">
      <div class="odds-column"><span>SPREAD</span><strong>${spread}</strong></div>
      <div class="odds-column"><span>FAVORITO</span><strong>${favorite}</strong></div>
    </section>`;

    if (!locked) return html;
    if (!reveal) return html + '<div class="notice game-note">Palpites individuais aguardando publicação.</div>';
    ...
    return html + `<details class="live-picks">
      <summary><b>Palpites e ${provisional ? 'pontuação provisória' : 'pontuação final'}</b></summary>
      <div class="table-wrap"><table><thead><tr><th>Participante</th><th>Palpite</th><th>Pontos</th></tr></thead><tbody>
      ${participants.map(item => {
        ...
        const label = pick ? `${item.winner}${item.difficulty ? ' · ' + item.difficulty : ''}` : 'Sem palpite';
        return `<tr><td>${item.name}</td><td>${label}</td><td class="live-points">${this.points(score.total)}</td></tr>`;
      }).join('')}
      </tbody></table></div>
      <p class="muted">${provisional ? 'Pontuação provisória, sujeita a alteração até o final.' : 'A pontuação oficial depende da apuração da rodada.'}</p>
    </details>`;
  },

  async refresh(week) {
    Bolao.Predictions.games = await Bolao.ESPN.games(week);
    const data = await this.load(week, Bolao.Predictions.games);

    document.querySelectorAll('#games .game-card').forEach((card, index) => {
      ...
      box.innerHTML = this.html(game, data[game.id], locked);
    });
  }
};

const nflLivePreviousWeekly = Bolao.Predictions.weekly.bind(Bolao.Predictions);
Bolao.Predictions.weekly = async function() {
  clearInterval(Bolao.LiveGames.timer);
  await nflLivePreviousWeekly();
  ...
  await Bolao.LiveGames.refresh(week);
};
