Bolao.SmartRefresh = {
  ...
  async run(week, manual) {
    ...
    try {
      ...
      if (Bolao.TeamRecordsV2) Bolao.TeamRecordsV2.apply();
      ...
      if (Bolao.PicksCompleteness) Bolao.PicksCompleteness.apply();
...
const nflSmartPreviousWeekly = Bolao.Predictions.weekly.bind(Bolao.Predictions);
Bolao.Predictions.weekly = async function() {
  clearTimeout(Bolao.SmartRefresh.timer);
  await nflSmartPreviousWeekly();
